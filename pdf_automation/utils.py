...
def url_to_pdf_path(abs_url: str, out_root: Path) -> Path:
	...
	path = parsed.path or "/"
	if path == "/":
		# Root becomes index.pdf at the root of out_root
		return out_root / Path("index.pdf")
	# Remove trailing slash if any
	if path.endswith("/"):
		path = path[:-1]
	# Build relative path mirroring site structure WITHOUT the domain
	segments = [sanitize_path_segment(s) for s in path.split("/") if s]
	# Use last segment as filename with .pdf
	rel_path = Path(*segments).with_suffix(".pdf") if segments else Path("index.pdf")
