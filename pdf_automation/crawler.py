...
from .utils import compute_sha256, extract_links, is_internal_url, normalize_url, url_to_pdf_path
    ...
    async def _worker(self, browser: Browser, queue: "asyncio.Queue[tuple[str, int]]") -> None:
        while True:
            ...
            try:
                ...
                # Skip via cache if unchanged; here we rely on content hash after render attempt
                # Render the page
                cleaned_html = await render_page_to_pdf(browser, url, pdf_path, self.config.out_root)
